...
public class MultiLoginPlugin extends JavaPlugin {
    ...
    // ---- 合并进来的 MC-MultiLogin-service（自包含验证，即原版 bukkit 的内嵌 hasJoined 引擎） ----
    ...
    public void onEnable() {
        ...
        // 2.5) 自包含验证配置（config.json，即合并进来的 MC-MultiLogin-service）。
        //      首次运行会自动生成 config.json，缺失即自举。
        ...
        // 自包含验证服务（合并进来的 MC-MultiLogin-service，内嵌 hasJoined 引擎）。
        ...
        if (verifyService != null) {
            getLogger().info("已启用内嵌 MC-MultiLogin-service 验证（method 数="
